...
export const metadata = {
  title: 'Manage Users - Admin - Cstyle',
...
async function fetchUsersForAdminPage(): Promise<User[]> {
  ...
  try {
    ...
    console.error(
      '[AdminUsersPage] CRITICAL: Error fetching users:',
      error.message
    );
    return [];
  }
...
export default async function AdminUsersPage() {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            User management is currently unavailable due to missing server
            configuration (MONGODB_URI or JWT_SECRET). Please check the server
            setup.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-muted-foreground">
            Admin user management features require essential environment
            variables to be set.
          </p>
        </CardContent>
      </Card>
  ...
  return <AdminUserClient initialUsers={initialUsers} />;
}
